...
const Story = () => {
    const [storyChallenge, setStoryChallenge] =
        useState<boolean>(false);

    const [selectedStory, setSelectedStory] =
        useState<StoryItem | null>(
            stories.length > 0
                ? (stories[0] as StoryItem)
                : null
        );
    ...
    const correctCount =
        selectedStory.questions?.filter(
                    ...
                    {(stories as StoryItem[]).map(
                        (story) => (
                            <div
                                ...
                                className={`rounded-xl flex justify-between w-[300px] h-[70px] mt-5 ml-5 pl-5 pt-2 cursor-pointer ${story.id ===
                                    selectedStory.id
                                    ? "bg-purple-300"
                                    : "bg-white"
                                    }`}
