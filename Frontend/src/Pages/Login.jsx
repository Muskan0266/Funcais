  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (data.setupComplete) {
        navigate("/"); // main page if already set up
      } else {
        navigate("/purpose"); // redirect new users to purpose page
      }
    } catch (error) {
    ...
    <div className="bg-white/80 p-10 mt-10 md:mt-25 rounded-2xl shadow-lg w-[80%] md:w-[60%] mx-auto max-w-[800px]">
      <div className="flex justify-center">
        <div className="text-center">
          ...
          {/* Social logins */}
          {[{ src: Google, label: "Google" }, { src: Apple, label: "Apple" }, { src: Facebook, label: "Facebook" }].map(
            (provider) => (
              <button
                key={provider.label}
              ...
              >
                <img className="h-5 w-5" src={provider.src} alt={provider.label} />
                <p>Continue with {provider.label}</p>
              </button>
            )
          )}
        </div>
